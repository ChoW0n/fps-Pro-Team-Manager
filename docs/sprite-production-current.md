...
> **외형 반려 후 수정:** 머리에서 팔이 솟는 시안은 사용자 반려. 지지팔을 안쪽으로 접고 위팔을 몸통/머리 뒤로 가리는 수정 적용. 이전 외형 품질 판정 철회, 사용자 미술 승인 대기. [상세](operator-shouldering-current.md)

> **최신 견착 보정:** [총기 중심선·견착 보정](operator-shouldering-current.md). 몸 안쪽 총열·어깨 포켓 접촉·C14 몸통 회전·반동 양손 동기화. 표시 총구를 시뮬레이션과 분리. 20무기 160방향, 64/128 정적 이미지, 타입·빌드 확인. 브라우저 미실행, 최종 원화/미술 승인 별도.

> **최신: [조립식 오퍼레이터 렌더링](operator-modular-current.md).** 총기 포함 통짜 시트 방향은 사용자 피드백으로 폐기됐다. 기존 시트는 이력으로 보존하지만 활성 중계 경로에서 사용하지 않는다. 하체·상체·팔·총기·손·군장을 분리하고 총기 접점에 양손을 조립하는 구조가 현재 기준이다.
