...
**실행기 호출·프로토콜 보강·오프라인 28개 테스트 완료 / 현재 Cloud 자식 프로세스에 ChatGPT 인증이 위임되지 않아 실제 모델 전환 미검증.**
...
기존 Node와 Codex 실행기를 사용한다. 2026-09-12 Cloud 환경의 Node v20.20.2에서도 오프라인 테스트를 통과했다. 새 npm 라이브러리는 추가하지 않는다. 사용자의 기기에 설치할 것을 전제로 하지 않으며, 기존 로그인된 Codex 작업 환경에서 실행한다. 실행기가 없거나 로그인이 없으면 실행을 중단한다.
