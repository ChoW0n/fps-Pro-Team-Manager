...
test('빗나간 발사도 실제 분산 방향의 탄착 지점을 보존',()=>{const missSample=utilitySample;const misses=missSample.events.filter(event=>event.type==='shot'&&!event.hit&&event.goal!=='wall-bang');assert(misses.length);for(const shot of misses){const snapshot=missSample.snapshots.find(s=>s.time===shot.time),target=snapshot.units.find(unit=>unit.id===shot.target);assert(target);assert(Math.hypot(shot.targetPosition.x-target.position.x,shot.targetPosition.y-target.position.y)>1);}});
...
test('실제 연막·카메라·수류탄 투척·폭발·회피가 엔진 사건에 존재',()=>{assert(!utilitySample.events.some(e=>e.time<15&&(e.type==='shot'||['grenade-thrown','smoke-thrown'].includes(e.goal))),'준비 중 공격 금지');for(const goal of ['camera-deployed','smoke-thrown','grenade-thrown','grenade-exploded'])assert([...sample.events,...utilitySample.events].some(event=>event.goal===goal),goal);assert(utilitySample.snapshots.some(snapshot=>snapshot.units.some(unit=>unit.goal.includes('수류탄 회피'))));});
...
test('전방 방패 상태가 실제 접촉·방향 스냅샷에 기록됨',()=>{const attacker=side('공격')[0],defender=side('수비')[0];defender.operator=OPERATORS.find(operator=>operator.callSign==='REUSS');
 ...
 const contact=new ShieldContact().run({attackers:[attacker],defenders:[defender],map:{...empty,defenderSpawn:{x:1320,y:1000},defenderSetups:[{id:'shield',position:{x:1320,y:1000},fallback:{x:1320,y:1000}}]},seed:41,maxSeconds:25});
...
test('미관측 적 추가가 접촉 중인 선수의 인원 판단·첫 발사 시점을 바꾸지 않음',()=>{
 // 22단위 발사 틈: 몸(지름24)은 통과하지 못하지만 몸 노출 광선은 통과합니다.
 // 공통 준비 구역에 첫 수비만 서고 나머지는 합법적인 외곽 대체 위치에서 시작합니다.
 class KnowledgeArena extends TacticalRealtimeSimulation {
  startPosition(unit,index,count){return unit.side==='공격'?{x:1000,y:1000}:index===count?{x:1600,y:1000}:{x:2600,y:200+(index-count)*65};}
  startFacing(unit){return unit.side==='공격'?0:Math.PI;}
 }
 function fixture(count){const attackers=side('공격').slice(2,3),defenders=side('수비').slice(0,count);for(const unit of [...attackers,...defenders])unit.player={...unit.player,aggression:0,aim:50,mastery:50,composure:50};return {attackers,defenders,map:{...empty,covers:[[1400,800,180,189],[1400,1011,180,189],[1580,1020,40,180],[1580,800,40,180],[1620,800,180,400]].map(([x,y,width,height],i)=>({id:'knowledge-cover-'+i,label:'발사 틈이 있는 검사 엄폐',rect:{x,y,width,height}})),defenderSpawn:{x:1600,y:1000},defenderSetups:defenders.map((_,i)=>({id:'knowledge-'+i,position:i?{x:2600,y:200+i*65}:{x:1600,y:1000},fallback:i?{x:2600,y:200+i*65}:{x:1600,y:1000}}))},seed:41,maxSeconds:18};}
 const one=new KnowledgeArena().run(fixture(1)),five=new KnowledgeArena().run(fixture(5));
 for(const u of five.snapshots[0].units)assert(engine.canStand(u.position,fixture(5).map),'추가 적도 지도 안 합법적 위치');
 const extraIds=five.snapshots[0].units.filter(u=>u.side==='수비').slice(1).map(u=>u.id);
 for(const s of five.snapshots)assert(extraIds.every(id=>!(s.observedBy[s.units[0].id]??[]).includes(id)),'비교용 추가 적은 실제로 미관측이어야 합니다');
 ...
 for(const snapshot of one.snapshots.filter(snapshot=>snapshot.time<=Math.min(17.5,(one.events.find(event=>event.type==='death')?.time??one.executionTime)-.1)))assert.deepEqual(snapshot.units[0],five.snapshots.find(other=>other.time===snapshot.time).units[0]);
});
test('설치 후 4대1: 네 선수의 분담 엄호와 로머의 실제 재진입·피격 순서',()=>{
 ...
 const terrain={...map,attackerRoutes:routes,defenderSpawn:{x:1840,y:2200},defenderSetups:[{id:'late-retake',label:'외곽 로머',position:{x:1840,y:2200},fallback:{x:1840,y:2150}}]};
 const rows=[];
