...
for(const text of ['방향','교전 태도','정보 우선순위','이 작전으로 출전'])assert(prep.includes(text),text);
...
fs.writeFileSync('validation/preparation-ui-contract.json',JSON.stringify({axes:3,liveDirectorCommands:false,removed:['선수별 수치','수동 편성','별동조','사이트 마지막 진입','경기 중 전술 지시']},null,2)+'\n');console.log('PASS 준비 세 축과 관전 전용 중계 계약');
