...
(async()=>{
 ...
 for(const weaponName of names)for(const stance of ['crawl','shield'])for(let facing=0;facing<8;facing++){
  ...
  for(const [index,a,b] of [[0,12,12],[1,8,9]]){near(distance(arms[index][0],arms[index][1]),a,weaponName+' '+stance+' upper');near(distance(arms[index][1],arms[index][2]),b,weaponName+' '+stance+' forearm');}
 ...
 for(const [i,label,extra] of [[0,'PRONE HK417',{locomotion:'crawl'}],[1,'PRONE MR73',{locomotion:'crawl',weaponName:'MR73'}],[2,'SHIELD HK417',{shieldRaised:true}],[3,'SHIELD USP',{shieldRaised:true,weaponName:'HK USP'}]]){
 ...
 fs.writeFileSync('validation/operator-equipment.json',JSON.stringify({scope:'Node Canvas; no browser',equipmentDirectionCases:cases,sidearmSprites:8,shieldSprites:2,checks:['rendered arm lengths','prone weapon retained','shield grip contact including recoil','display muzzle matches draw transform','shield uniform scale','shield clears muzzle axis','legacy sidearm simulation origin','frozen inputs','down/dead unarmed'],limits:['New art awaits user feedback','No secondary weapon selection logic','Shield raised reload/throw hand transitions not added','No real-device or performance gate claim']},null,2)+'\n');
