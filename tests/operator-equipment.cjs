...
(async()=>{
 ...
 for(const weaponName of names)for(const stance of ['crawl','shield'])for(let facing=0;facing<8;facing++){
  ...
  const ctx=new Proxy(native,{get(target,key){const value=target[key];if(typeof value!=='function')return value;return(...args)=>{
   ...
   if(key==='quadraticCurveTo'&&points.length===2)points[1]={x:args[0],y:args[1]};
        if(key==='stroke'&&points.length===3&&Math.abs(target.lineWidth-5.2)<1e-5)arms.push(points.slice());
  ...
  for(const [index,a,b] of [[0,stance==='crawl'?(m.kind==='pistol'?9:['bolt','marksman'].includes(m.kind)?14:16):8,stance==='crawl'?(m.kind==='pistol'?10:['bolt','marksman'].includes(m.kind)?14:16):10],[1,stance==='crawl'?10:8,stance==='crawl'?11:9]]){near(distance(arms[index][0],arms[index][1]),a,weaponName+' '+stance+' upper');near(distance(arms[index][1],arms[index][2]),b,weaponName+' '+stance+' forearm');}
  near(distance(arms[1][2],{x:m.triggerHand.x-1.2,y:m.triggerHand.y}),0,'반동 방아쇠 접점');
  if(stance==='crawl'&&m.kind!=='pistol'){assert(m.supportHand.x>m.triggerHand.x,'지지손은 방아쇠손 앞쪽');assert(arms[0].every(p=>p.y<m.muzzle.y),'왼팔은 총기 왼쪽에서 하부 파지');assert(arms[1].every(p=>p.y>m.muzzle.y),'오른팔은 총기 오른쪽에서 방아쇠 파지');}
  ...
  if(stance==='crawl'||weaponPart(weaponName).file.endsWith('-v2.png')){
 ...
 fs.writeFileSync('validation/operator-equipment.json',JSON.stringify({scope:'Node Canvas; no browser',equipmentDirectionCases:cases,sidearmSprites:8,shieldSprites:2,checks:['rendered arm joint controls','prone weapon retained','underhand draw order','forward support grip','shield arm side separation','shield grip contact including recoil','display muzzle matches draw transform','shield uniform scale','shield clears muzzle axis','legacy sidearm simulation origin','frozen inputs','down/dead unarmed'],limits:['New art awaits user feedback','No full draw/holster animation','No real-device or performance gate claim']},null,2)+'\n');
