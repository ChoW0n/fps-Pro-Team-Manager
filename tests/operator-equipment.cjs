...
(async()=>{
 ...
 for(const weaponName of names)for(const stance of ['crouch','shield'])for(let facing=0;facing<8;facing++){
  ...
  const ctx=new Proxy(native,{get(target,key){const value=target[key];if(typeof value!=='function')return value;return(...args)=>{
   ...
   if(key==='quadraticCurveTo'&&points.length===2)points[1]={x:args[0],y:args[1]};
        if(key==='stroke'&&points.length===3&&Math.abs(target.lineWidth-(stance==='shield'?5.2:4.8))<1e-5)arms.push(points.slice());
 ...
 fs.writeFileSync('validation/operator-equipment.json',JSON.stringify({scope:'Node Canvas; no browser',equipmentDirectionCases:cases,sidearmSprites:8,shieldSprites:2,checks:['rendered arm joint controls','legacy crawl uses crouch','underhand draw order','shield arm side separation','shield grip contact including recoil','display muzzle matches draw transform','shield uniform scale','shield clears muzzle axis','legacy sidearm simulation origin','frozen inputs','down/dead unarmed'],limits:['New art awaits user feedback','No full draw/holster animation','No real-device or performance gate claim']},null,2)+'\n');
