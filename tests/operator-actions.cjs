...
const render=(unit,time=1,motion={},reduced=false)=>{const c=createCanvas(220,180),ctx=c.getContext('2d');ctx.translate(110,90);ctx.scale(2.3,2.3);const before=JSON.stringify(unit);paintMinimalOperator(ctx,unit,time,undefined,()=>{},reduced,motion);assert.equal(JSON.stringify(unit),before,'표시가 실제 탄약·상태를 수정하지 않음');return c;};
...
for(const [row,weapon] of ['L119A2','FN P90'].entries()){
 ...
 assert.notEqual(bytes(start),bytes(middle),'장전 중 탄창과 지지 손이 이동함');
...
fs.writeFileSync('validation/operator-actions.json',JSON.stringify({cases,checks:'reload progress and completion, deterministic paused frame, event-bound throw, expired/future events, down-state blocking, install posture, reduced motion, no unit mutation',limits:'Canvas state fixtures; not a real-device interaction test'},null,2)+'\n');
