...
const report={map:map.id,indoorSamples:points.length,lineOfSight:{longestMetres:longest,pair,openPairs:open,over25mPairs:over,over25mPercent:100*over/Math.max(1,open),targetLongestMetres:25,targetOverPercent:10},oversizedRooms:map.rooms.filter(r=>r.kind!=='yard'&&(r.rect.width>320||r.rect.height>320)).map(r=>({id:r.id,widthMetres:r.rect.width/metre,heightMetres:r.rect.height/metre})),siteEntrances:map.sites.map(s=>({site:s.id,portals:map.portals.filter(p=>(s.roomIds??[s.roomId]).includes(p.fromRoom)||(s.roomIds??[s.roomId]).includes(p.toRoom)).map(p=>p.id)})),spawnStraightLineMetres:{min:Math.min(...spawnDistances),max:Math.max(...spawnDistances),ratio:Math.max(...spawnDistances)/Math.min(...spawnDistances)},verticalConnections:map.stairs?.length??0};
