...
class LocalImage extends Image {set src(url){this.file=url;super.src=app+'/public'+url;}}
...
const rows=[];
async function render(goal,age,seenBy=['공격'],floor=0){
 const canvas=createCanvas(1000,700),ctx=canvas.getContext('2d'),calls=[];calls.rings=[];
 const arc=ctx.arc.bind(ctx);ctx.arc=(...args)=>{if(['#e89989','#9ecdb9','#e6c775'].includes(ctx.strokeStyle.toLowerCase()))calls.rings.push(args);return arc(...args);};
