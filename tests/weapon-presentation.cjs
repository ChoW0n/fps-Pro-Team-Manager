...
OPERATORS.forEach((op,i)=>{
 ...
 for(let angle=0;angle<8;angle++){
  ...
  const bare=createCanvas(128,128),bc=bare.getContext('2d');skipWeapon=true;paintMinimalOperator(bc,unit,1,undefined,()=>{});skipWeapon=false;
...
for(const boundary of [Math.asin(.72),Math.PI/2,Math.PI-Math.asin(.72),Math.PI,Math.PI+Math.asin(.72),Math.PI*1.5,Math.PI*2-Math.asin(.72),Math.PI*2]){
 const render=facing=>{const c=createCanvas(128,128);paintMinimalOperator(c.getContext('2d'),{id:'continuity',callSign:'MAGPIE',side:'공격',weaponName:'L119A2',position:{x:64,y:64},velocity:{x:0,y:0},facing,alive:true,action:'hold'},1,undefined,()=>{});return c.getContext('2d').getImageData(0,0,128,128).data;};
