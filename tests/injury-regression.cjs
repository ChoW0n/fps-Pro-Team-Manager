...
test('실제 소생 사건은 구조자의 정지 시간·대상 회복과 일치',()=>{let count=0;for(const seed of [1,7,19,41]){const result=new TacticalRealtimeSimulation().run(input(seed));for(const e of result.events.filter(e=>e.type==='revive')){count++;const now=result.snapshots.find(s=>s.time===e.time),prior=result.snapshots.find(s=>s.time===Math.round((e.time-.1)*10)/10);const helper=prior.units.find(u=>u.id===e.actor),target=now.units.find(u=>u.id===e.target);assert(helper.reviving);assert(e.time-helper.reviving.startedAt>=4-1e-6);assert(target.alive&&!target.downed&&target.hp===20);}}assert(count>0);});
console.log(`${passed} injury checks passed`);
fs.writeFileSync(require('node:path').join(__dirname,'../validation/injury-results.json'),JSON.stringify({passed,total:12,seeds:[1,7,19,41],scope:'Injury unit checks and real engine deterministic fixtures; not browser play or Siege rules verification'},null,2)+'\n');
