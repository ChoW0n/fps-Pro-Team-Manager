...
const {NAMSAN_MAP:map}=require('../artifacts/draft-order-player-generator/src/domain/tacticalMaps.ts');
...
ctx.fillStyle='#eee9d7';ctx.font='bold 34px sans-serif';ctx.fillText('남산 중계관 / 1F',32,56);ctx.font='18px sans-serif';ctx.fillStyle='#b8c9c3';ctx.fillText('실제 지도 렌더러 · 가상 시설 · 40단위 = 1m',35,85);
ctx.save();ctx.translate(1060,110);ctx.scale(.14,.14);paintBattleMap(ctx,map);ctx.fillStyle='#FFD66C';for(const route of map.attackerRoutes.slice(0,5)){const p=route.points[0];ctx.beginPath();ctx.arc(p.x,p.y,38,0,Math.PI*2);ctx.fill();}ctx.restore();
ctx.fillStyle='#eee9d7';ctx.font='bold 24px sans-serif';ctx.fillText('관광 · 방송 · 설비',1070,590);ctx.font='18px sans-serif';
for(const [i,line] of ['실내 576㎡ · 방 16개','A  송출 / 관제','B  전원 / 배전','외부 진입 5개 · 저상 2개','표본 최장 사선 24.1m','2층·수직 해치는 후속 단계'].entries())ctx.fillText(line,1070,640+i*42);
fs.writeFileSync('validation/namsan-map-review.png',canvas.toBuffer('image/png'));console.log('PASS 공통 지도 렌더');
