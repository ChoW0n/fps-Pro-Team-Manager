...
(async()=>{
 ...
 await page.goto(new URL('art-review/operator-topdown-v1/',process.env.DRAFT_ORDER_URL??'http://localhost:5173/draft-order-player-generator/').href);await page.waitForLoadState('networkidle');
 ...
 for(const weapon of ['p90','hk416','c14']){await page.locator('#weapon').selectOption(weapon);for(const angle of ['0','90','180','270']){await page.locator('#angle').evaluate((el,value)=>{el.value=value;el.dispatchEvent(new Event('input',{bubbles:true}));},angle);}await page.screenshot({path:out+'/operator-'+weapon+'.png',fullPage:true});}
 await page.evaluate(async()=>{
 const audio=await import('/draft-order-player-generator/src/components/matchAudio.ts');
 const Native=window.AudioContext,sources=window.__audioSources=[],oscillators=window.__audioOscillators=[];
 ...
 audio.setMatchAudioMuted(false);const button=document.createElement('button');button.id='start-audio-check';button.textContent='오디오 검사 시작';button.onclick=()=>audio.unlockMatchAudio();document.body.append(button);return {ready:true};
 });
 await page.locator('#start-audio-check').click();
 await page.waitForTimeout(100);
 const result=await page.evaluate(async()=>{
 const audio=await import('/draft-order-player-generator/src/components/matchAudio.ts');
 const sources=window.__audioSources,oscillators=window.__audioOscillators;
 ...
 fs.writeFileSync(out+'/browser-results.json',JSON.stringify(result,null,2)+'\n');console.log('PASS: 3 weapon previews; 6 audio buffer sources after click; no pitched oscillators; no page errors');
