...
class LocalImage extends Image {set src(url){const name=url.split('/').at(-1),png=path.join(decoded,name.replace(/\.webp$/,'.png'));super.src=fs.readFileSync(fs.existsSync(png)?png:app+'/public/operators/'+name);}}
...
(async()=>{for(const [name,width,height] of [['desktop',1280,720],['phone',390,844],['phone-landscape',844,390],['downed',1280,720],['collier-downed',1280,720]]){
 ...
 const context=canvas.getContext('2d'),originalDraw=context.drawImage.bind(context);let spriteDraws=0,downedPoseDraws=0;
 // 실제 인물 drawImage가 호출되지 않으면 빈 화면을 통과시키지 않습니다.
 context.drawImage=(source,...args)=>{if(source instanceof LocalImage){spriteDraws++;if(args.length===8&&args[0]===910&&args[1]===565)downedPoseDraws++;const transform=context.getTransform();assert(Math.abs(Math.hypot(transform.a,transform.b)-Math.hypot(transform.c,transform.d))<1e-6,'전신 원화의 비균등 압축 금지');}return originalDraw(source,...args);};
 ...
 const casualty=u=>u.side==='공격'&&u.downed&&(name!=='collier-downed'||u.callSign==='COLLIER');
 const injurySnapshot=name.endsWith('downed')?result.snapshots.find(s=>s.units.some(casualty)):undefined;
 if(name.endsWith('downed'))assert(injurySnapshot,'실제 다운 장면 필요');
