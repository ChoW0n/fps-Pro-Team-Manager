...
class LocalImage extends Image {set src(url){const name=url.split('/').at(-1),png=path.join(decoded,name.replace(/\.webp$/,'.png'));this.assetName=name;super.src=url.includes('/operators/effects/')?app+'/public/operators/effects/'+name:url.includes('/effects/')?app+'/public/effects/'+name:fs.existsSync(png)?png:app+'/public/operators/'+(url.includes('/survivor/')?'survivor/':url.includes('/weapons/top/')?'weapons/top/':url.includes('/weapons/')?'weapons/':'')+name;}}
