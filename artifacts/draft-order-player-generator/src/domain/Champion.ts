...
export type ChampionPositionFit = Record<Position, number>;
...
function derivePositionFits(
  ...
  const fits: ChampionPositionFit = {
    TOP: role === 'TANK' ? 78 : role === 'DAMAGE' ? 70 : 48,
    JUNGLE: engagement === 'DIVE' ? 78 : 62,
    MID: range === 'AOE' ? 76 : role === 'DAMAGE' ? 70 : 54,
    ADC: range === 'SINGLE' && role === 'DAMAGE' ? 82 : range === 'AOE' ? 68 : 42,
    SUPPORT: role === 'UTILITY' ? 84 : role === 'TANK' ? 72 : 44,
  };
  if (engagement === 'POKE') {
    fits.MID += 8;
    fits.ADC += 7;
  ...
  const difficultyPenalty = (difficulty - 3) * 3;
  return Object.fromEntries(
    (Object.keys(fits) as Position[]).map((position) => [
      position,
      Math.max(0, Math.min(100, Math.round(fits[position] - difficultyPenalty))),
    ]),
  ) as ChampionPositionFit;
}
...
export class Champion {
  ...
  public readonly positionFits: ChampionPositionFit;
  public readonly positionMetaBonuses: ChampionPositionFit;
  ...
  constructor(
    ...
    this.positionFits = derivePositionFits(role, engagement, range, timing, difficulty);
    this.positionMetaBonuses = { TOP: 0, JUNGLE: 0, MID: 0, ADC: 0, SUPPORT: 0 };
  ...
  public getPositionFit(position: Position): number {
    return Math.max(0, Math.min(100, this.positionFits[position] + this.positionMetaBonuses[position]));
  }
}
...
function printLevel18UltimateDamageValidation(): void {
  ...
  const tankArmor = tank.armorBase + tank.armorGrowth * (level - 1);

  (['SINGLE', 'AOE'] as const).forEach((range) => {
    const engagement: Engagement = range === 'SINGLE' ? 'DIVE' : 'POKE';
    const dealer = deriveCombatStats('DAMAGE', engagement, range, 'LATE', 3);
    const scaling = deriveSkillScaling(
      'DAMAGE',
      range,
      3,
      dealer.basicRange,
      dealer.ultimateRange,
      dealer.basicCooldown,
      dealer.ultimateCooldown,
    ).ultimate;
    const attack = dealer.attackBase + dealer.attackGrowth * (level - 1);
    const rawDamage = scaling.baseDamage
      + scaling.damagePerLevel * (scaling.maxLevel - 1)
      + attack * scaling.attackCoefficient;
    const damageAfterArmor = rawDamage * 100 / (100 + tankArmor);
    const percentage = damageAfterArmor / tankHealth * 100;
    console.log(
      `[스킬 피해량 검증] 18레벨 딜러 ${range === 'SINGLE' ? '단일' : '광역'} 궁극기 3레벨: `
      + `18레벨 탱커 최대 체력의 ${percentage.toFixed(1)}% `
      + `(${percentage >= 30 && percentage <= 60 ? '통과' : '실패'} · 기준 30~60%)`,
    );
  });
}
...
export function getChampionsByPosition(position: Position): Champion[] {
  return [...CHAMPIONS].sort((left, right) =>
    right.getPositionFit(position) - left.getPositionFit(position)
