...
export function resolveElectronicCounters(gadgets:RealtimeGadget[],time:number,clearLine:(a:TacticalPoint,b:TacticalPoint)=>boolean):RealtimeEvent[] {
  ...
  for(const projectile of [...gadgets]){
    ...
    const interceptor=gadgets.find(gadget=>gadget.kind==='interceptor'&&gadget.side!==projectile.side&&gadgetActive(gadget,time)&&(gadget.charges??0)>0
  ...
  for(const pulse of [...gadgets].filter(gadget=>gadget.kind==='emp'&&gadgetActive(gadget,time))){
    ...
    for(const device of gadgets.filter(electronic))if(distance(device.position,pulse.position)<=pulse.radius)device.disabledUntil=Math.max(device.disabledUntil??0,time+8);
