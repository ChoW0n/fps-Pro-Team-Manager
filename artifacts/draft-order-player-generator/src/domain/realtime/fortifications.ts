...
export function battlefieldMap(base: TacticalMapDefinition, breaches: readonly {wallId:string;position:{x:number;y:number};width:number;hard?:boolean}[] = [], fortifications: readonly Fortification[] = []): TacticalMapDefinition {
  ...
  return {...base,walls:breaches.reduce((current,hole)=>breachWalls(current,hole.wallId,hole.position,hole.width,hole.hard),walls),
