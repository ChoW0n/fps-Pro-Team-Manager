...
/** 감독은 준비한 작전의 결과를 관전하며 선수·시점·재생 속도만 선택합니다. */
export function TacticalRoundLive({input,map=NAMSAN_MAP,roundNumber=1,directorSide='공격',score=[0,0],onComplete}:TacticalRoundLiveProps):ReactElement {
  ...
  const [decisionFeed,setDecisionFeed]=useState<DecisionFeedItem[]>([]);
  ...
  const seenDecisions=useRef(new Map<string,string>());
  ...
  useEffect(()=>{
    ...
    setSelectedId(first?.[0]??null);setTick(null);setEvents([]);setResult(null);setError('');setPaused(false);setDecisionFeed([]);heardEvents.current.clear();seenDecisions.current.clear();controls.current.paused=false;
    /** 실시간 틱과 최종 통지를 분리해 라운드 점수를 한 번만 기록합니다. */
    const receiveTick=(next:RealtimeTick):void=>{
      if(stopped)return;
      setTick(next);setEvents(previous=>[...previous,...next.events].slice(-120));
      const changed:DecisionFeedItem[]=[];
      for(const unit of next.snapshot.units) {
        if(unit.side!==directorSide||!unit.alive||!unit.decision)continue;
  ...
  return <section className={`match-broadcast phase-${phase} ${mode==='full'?'is-tactical':''}`} aria-label="DRAFT ORDER 경기 중계" data-version="round-flow-mobile-20260913">
    ...
    <aside className="cast-decisions" aria-label="선수 판단 피드"><AnimatePresence initial={false}>{decisionFeed.map(item=><motion.p key={item.id} initial={reducedMotion?false:{opacity:0,y:8}} animate={{opacity:1,y:0}} exit={{opacity:0}} transition={{duration:reducedMotion?0:.16}}><small>{clock(item.time)}</small>{item.text}</motion.p>)}</AnimatePresence></aside>
