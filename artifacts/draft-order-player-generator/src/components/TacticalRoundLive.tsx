...
export function TacticalRoundLive({input,map=BREACHLINE_MAP,roundNumber=1,directorSide='공격',score=[0,0],onComplete}:TacticalRoundLiveProps):ReactElement {
  ...
  const commands=useRef<TacticalDirectorCommand[]>([]);
  const [orderStatus,setOrderStatus]=useState(''),[orderRoute,setOrderRoute]=useState(0);
  ...
  useEffect(()=>{
    commands.current=[];setOrderStatus('');setOrderRoute(0);
    ...
    const receiveTick=(next:RealtimeTick):void=>{if(stopped)return;setTick(next);const order=next.events.findLast(event=>event.actor==='director'&&event.side===directorSide);if(order)setOrderStatus('전달됨 · '+order.goal);setEvents(previous=>[...previous,...next.events].slice(-240));};
    const receiveResult=(value:TacticalRealtimeResult):void=>{if(stopped||reported)return;reported=true;commands.current=[];setOrderStatus('라운드 종료');setResult(value);complete.current?.(value);};
    ...
    }else fallbackStop=startRoundPlayback(new TacticalRealtimeSimulation(map).createSession(input),()=>controls.current,receiveTick,receiveResult,()=>commands.current.shift());
  ...
  const kills=known.filter(event=>['death','downed','revive'].includes(event.type)&&time-event.time<6).slice(-4);
  ...
  /** 감독이 고른 지시만 같은 팀의 실제 엔진에 전달합니다. */
  function issue(mode:TacticalDirectorCommand['mode'],label:string):void {
    if(result||error||!tick)return;
    const command:TacticalDirectorCommand={side:directorSide,mode,label,...(mode==='route'?{routeIndex:orderRoute}:{})};
    if(workerRef.current)workerRef.current.postMessage({type:'command',command});else commands.current.push(command);
    setOrderStatus((paused?'재생 대기 · ':'전달 대기 · ')+label);
  }
  return <section className={`match-broadcast ${mode==='full'?'is-tactical':''}`} aria-label="DRAFT ORDER 경기 중계" data-version="personal-broadcast-20260911">
    <BroadcastCanvas tick={tick} events={events} map={map} side={directorSide} playerNames={new Map([...participants].map(([id,unit])=>[id,unit.player.nickname]))} selectedId={selectedId} mode={mode} speed={speed} paused={paused} onSelect={select} onFocus={setFocusedId}/>
    ...
    <div className="cast-killfeed" aria-label="전투 피드">{kills.map((event,index)=><div key={`${event.time}-${index}`}><b>{killerName(event)}</b><span>{event.type==='downed'?'다운':event.type==='revive'?'소생':event.hitRegion==='head'?'헤드샷':'처치'}</span><strong>{playerName(event.target)}</strong></div>)}</div>
    ...
    <nav className="director-controls" aria-label="감독 전술 지시">
      <button disabled={!tick||Boolean(result)||Boolean(error)} onClick={()=>issue('push','진입')}>진입</button>
      <button disabled={!tick||Boolean(result)||Boolean(error)} onClick={()=>issue('hold','대기')}>대기</button>
      <button disabled={!tick||Boolean(result)||Boolean(error)} onClick={()=>issue('retreat','후퇴')}>후퇴</button>
      {directorSide==='공격'&&<><select aria-label="변경할 진입로" value={orderRoute} onChange={event=>setOrderRoute(Number(event.target.value))}>{map.attackerRoutes.map((_,index)=><option key={index} value={index}>진입로 {index+1}</option>)}</select><button disabled={!tick||Boolean(result)||Boolean(error)} onClick={()=>issue('route',`경로 변경 · ${orderRoute+1}`)}>경로 변경</button></>}
      <output aria-live="polite">{orderStatus}</output>
    </nav>
