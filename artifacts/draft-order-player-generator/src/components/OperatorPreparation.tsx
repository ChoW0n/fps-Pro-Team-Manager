...
export function OperatorPreparation({ homeTeam, awayTeam, onStart, onBack }: {
  homeTeam: Team; awayTeam: Team; onStart: (input: TacticalRealtimeSimulationInput) => void; onBack: () => void;
}) {
  ...
  const draft=useMemo(()=>{
    try { return {home:completeOperatorDraft(homeTeam,'공격',manual),away:completeOperatorDraft(awayTeam,'수비',[null,null,null,null,null]),error:''}; }
    catch(cause){return {home:[...manual],away:[] as OperatorSelection,error:cause instanceof Error?cause.message:'편성을 확인해 주세요.'};}
  },[homeTeam,awayTeam,manual]);
  const player=homeTeam.players[selectedPlayer];
  const choices=availableOperators(homeTeam,selectedPlayer,'공격');
  ...
  function choose(callSign: string): void {
    ...
    try { completeOperatorDraft(homeTeam,'공격',next);setManual(next);setError(''); }
  ...
  function start(): void {
    try {
      onStart({attackers:confirmOperatorDraft(homeTeam,'공격',draft.home),defenders:confirmOperatorDraft(awayTeam,'수비',draft.away),
        seed:20260910,maxSeconds:180,targetSite:site,scoutPlan:{indices:[...scouts],seconds,entryRoute}});
  ...
  return <section className="operator-preparation" aria-label="오퍼레이터 편성과 작전 준비">
    <header className="op-prep-heading"><div><small>DRAFT ORDER / OPERATION ROOM</small><h1>다섯 명의 준비가 경기에서 드러납니다.</h1><p>습득한 오퍼레이터를 배정하고, 선발조와 진입 방향을 정해 주세요.</p></div><span>BREACHLINE<br/><b>5 vs 5 · 폭탄전</b></span></header>
    <div className="op-prep-layout">
      <section className="op-prep-roster" aria-label="공격팀 선수 편성"><h2>{homeTeam.name}<small>공격팀 / 내 편성</small></h2>
        {homeTeam.players.map((member,index)=><div key={index} className={`op-prep-slot ${selectedPlayer===index?'is-selected':''}`}>
          ...
          <label><input type="checkbox" checked={scouts.includes(index)} disabled={!scouts.includes(index)&&scouts.length===3} onChange={()=>toggleScout(index)}/>선발조</label>
        </div>)}
      </section>
      ...
      <section className="op-prep-plan" aria-label="감독 작전"><h2>경기 작전</h2>
        <label>목표 사이트<select value={site} onChange={event=>setSite(event.target.value as 'A'|'B')}>{BREACHLINE_MAP.sites.map(target=><option key={target.id} value={target.id}>{target.label}</option>)}</select></label>
        ...
        <div className="op-prep-opponent"><small>수비팀 / {awayTeam.name}</small><p>{draft.away.filter(Boolean).join(' · ')}</p></div>
      </section>
    </div>
