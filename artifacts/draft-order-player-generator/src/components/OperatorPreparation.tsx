...
import { NAMSAN_MAP } from '../domain/tacticalMaps';
...
export function OperatorPreparation({homeTeam,awayTeam,onStart,onBack,homeSide='공격',seed=20260910,history=[]}: {
  ...
  const route=NAMSAN_MAP.attackerRoutes[entryRoute];
  ...
  return <section className="operator-preparation command-room" aria-label="오퍼레이터 편성과 작전 준비">
    <header className="op-prep-heading"><div><small>PREPARE / {homeSide==='공격'?'ATTACK':'DEFEND'}</small><h1>이번 라운드, 어떻게 풀어갈까요?</h1><p>작전은 여기서 정하고, 경기에서는 선수의 판단을 지켜봅니다.</p></div><span>{homeTeam.name}<br/><b>{homeSide} · 남산 중계관</b></span></header>
    <div className="command-layout">
      <section className="command-map-panel" aria-label="작전 지도 미리보기"><div className="command-map"><TacticalBattlefield map={NAMSAN_MAP} units={[]} operators={new Map()} events={[]} time={0} selectedId={null} onSelect={()=>{}} miniature/>
        <svg className="command-route" viewBox={`0 0 ${NAMSAN_MAP.width} ${NAMSAN_MAP.height}`} aria-label="선택한 진입 구간과 목표">
          {homeSide==='공격'&&secondaryIndices.length>0&&<polyline points={NAMSAN_MAP.attackerRoutes[secondaryRoute].points.slice(0,4).map(point=>`${point.x},${point.y}`).join(' ')} fill="none" stroke="#FFC53D" strokeWidth="16" strokeDasharray="12 16"/>}
        ...
        <div className="command-routes">{NAMSAN_MAP.attackerRoutes.slice(0,5).map((route,index)=><button key={route.id} aria-pressed={entryRoute===index} onClick={()=>chooseEntry(index)}>{route.label.replace(' 진입','')}</button>)}</div>
      </section>
